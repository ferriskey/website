...
### Anomaly Alerting
Combine SeaWatch with [Webhooks](/en/modules/webhooks/overview) to trigger alerts on suspicious patterns, multiple `login_failure` events from the same IP, `client_secret_rotated` events outside of maintenance windows, or `realm_config_changed` events from unexpected actors.
