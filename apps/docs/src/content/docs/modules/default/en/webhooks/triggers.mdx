...
Webhook triggers overlap with [SeaWatch event types](/modules/default/en/seawatch/event-types) but are not identical. SeaWatch tracks security audit events (login success/failure). Webhooks track resource lifecycle events. Use both for complete coverage.
