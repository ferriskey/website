...
Signing in *through* an external SAML provider is what is planned here. FerrisKey acting *as* a SAML identity provider, so other applications can trust it, is shipped: see the [SAML module](/en/modules/saml/overview).
:::
