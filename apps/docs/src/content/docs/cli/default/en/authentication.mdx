...
::::step-group
:::step{title="Reuse a login session"}
If `credentials.toml` exists and its `(server_url, realm, client_id)` matches the active context, the saved `access_token` is reused.
:::
