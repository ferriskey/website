...
A claim is a key-value pair in a JWT's payload. Some claims have universally agreed meanings; others are application-specific. Knowing which is which keeps your tokens interoperable.
...
Validators must always check `exp`, `nbf`, `iss`, and `aud`. Skipping any of these is a known class of security bug.
