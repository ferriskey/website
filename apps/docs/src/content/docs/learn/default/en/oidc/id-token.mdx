...
The ID Token is OIDC's contribution to OAuth2. It is a JWT issued alongside the access token, and it has exactly one job: tell the client who the user is.

It is not for calling APIs. It is for the client to read.
...
This is what a competent OIDC client library does every time:
...
6. **Check `nonce`** matches what was sent in the authorize request.
7. (If present) **Check `c_hash` / `at_hash`** against the code and access token.

Skipping any of these is a security bug. Use a vetted library; never roll this from scratch.
