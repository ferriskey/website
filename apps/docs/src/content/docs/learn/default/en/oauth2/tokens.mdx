...
Two pieces of paper come out of that envelope: an access token and a refresh token. They do different jobs.
...
If an access token leaks, the attacker has access until it expires. Short lifetimes shrink that window from "forever" to "a few minutes." The trade-off: the client needs a way to renew them silently. That is what refresh tokens are for.
