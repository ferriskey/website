...
The images above track the latest tag. For reproducible deployments, pin an explicit tag (for example `ghcr.io/ferriskey/ferriskey-api:v0.7.0`) on both the `api` and `db-migrations` services, so they always run the same binary and schema.
:::
...
The password grant above is fine for a quick API test. Real applications should use browser-based SSO with the OpenID Connect authorization code flow.
