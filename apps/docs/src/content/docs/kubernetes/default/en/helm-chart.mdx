...
|---|---|---|
| `database.host` | None | PostgreSQL host |
| `database.port` | `5432` | PostgreSQL port |
| `database.name` | `ferriskey` | Database name |
| `database.user` | `ferriskey` | Database user |
| `database.passwordSecret.name` | None | Kubernetes Secret name containing the password |
| `database.passwordSecret.key` | None | Key within the Secret |
...
| `common.serviceAccount.name` | None | ServiceAccount name override |
